...
def run_forecast_with_adjustments(major, cpi_values, gdp_values):
    data_filtered = data[data['Major'] == major]
    data_agg = data_filtered.groupby('Year').agg({'Income': 'mean', 'CPI': 'mean', 'GDP': 'mean'}).reset_index()
    ...
    # Create exogenous variables (CPI and GDP)
    exog_vars = np.column_stack([cpi_ts, gdp_ts])
    
    # Fit the ARIMA model
    model = SARIMAX(income_ts, exog=exog_vars, order=(1, 1, 1)).fit(disp=False)
    
    # Forecast for the next 4 years using the student inputs
    forecast = model.get_forecast(steps=4, exog=np.column_stack([cpi_values, gdp_values]))
...
income_ts, forecast_mean = run_forecast_with_adjustments(major, cpi_values, gdp_values)
